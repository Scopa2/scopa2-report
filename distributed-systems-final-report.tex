...
\author{
    Marco Coppola \\ \emailaddr{marcocoppola02.mc@gmail.com}
    \and
...
Scopa2 does not fit neatly into a single one of the canonical
architectural styles; it is a deliberate hybrid of three.

The \emph{layered} or \emph{three-tier} structure organises every region
as a stack of presentation, application, and data. Inside the
application tier the controllers translate HTTP requests into domain
actions; the model layer encapsulates the game engine; the persistence
layer is reached only through Eloquent ORM. The advantage of this strict
separation is clear modularity --- the engine can be unit-tested in
isolation, the controllers replaced wholesale --- at the cost of
crossing a few extra function calls on every request.

The \emph{event-based} style governs the realtime fan-out. Once a move
has been validated and persisted, the server emits a domain event
(\texttt{GameStateUpdated}, \texttt{RoundFinished}, \texttt{GameFinished},
\texttt{MatchFound}) that is published on the appropriate channel and
consumed by every subscribed client. This decouples the controllers from
the clients in both time and space, and it is what \citet{eugster2003many}
call the \emph{many-faced} pattern of publish/subscribe: the producer
need not know who consumes the event, and consumers need not know who
produced it.

The \emph{shared dataspace} style emerges in the matchmaking subsystem,
where the queue lives as a Redis sorted set on which any backend pod can
operate. The ELO rating acts as the score, and the matching logic
inspects the dataspace rather than holding any in-memory state itself.
This is the closest analogue in our system to a Linda-style tuple space:
every pod \emph{reads} the queue, every pod can \emph{write} to it, and
nobody owns it.

We rejected the \emph{object-based} style described by
\citet{birrell1984implementing} for the same reasons the course critiques
it: fine-grained remote method calls between the application and the
database would have multiplied chatty round-trips, and we wanted the
freedom to swap the underlying database, the Redis cluster, or the
realtime broker without rewriting the controllers.

\subsection{Infrastructure}\label{infrastructure}

\figph{topology}{3.4cm}{Global symmetric topology. Each region (EU,
...
\figph{class-engine}{3.2cm}{Class diagram of the in-memory game engine.
\texttt{ScopaEngine} owns a \texttt{GameState} which aggregates a deck,
a table, a \texttt{PlayerCollection} of two \texttt{PlayerState}s, a
shared \texttt{Scores} object, a per-card \texttt{Mutations} map, and
three \emph{Santi} drawn from the shop. The engine receives a stream of
\texttt{GameEvent}s (PGN-like notation) and re-applies them to produce a
deterministic state.}
...
requests against a small REST surface: \texttt{POST /api/auth/register},
\texttt{POST /api/auth/login}, \texttt{POST /api/matchmaking/join},
\texttt{POST /api/matchmaking/leave}, \texttt{GET /api/matchmaking/status},
\texttt{POST /api/games}, \texttt{POST /api/games/\{id\}/join},
\texttt{GET /api/games/\{id\}} and the central
\texttt{POST /api/games/\{id\}/action} that submits a move. All
endpoints carry a \texttt{Bearer} token (Sanctum) and a custom
\texttt{player\_secret} header derived from the user id; the
combination is what makes a single request unambiguously
identity-bound. Responses are JSON.
...
\figph{seq-matchmaking}{3.0cm}{Sequence of the discovery and
...
\figph{seq-action}{3.6cm}{Sequence of a single in-game action. The
...
\figph{state-match}{2.8cm}{State machine of a match. A game is born in
...
\begin{lstlisting}[language={[5.2]Lua},caption={Atomic matchmaking
section --- guarantees that no two pods can ever extract the same player
from the queue, regardless of how many pods are running.},label={lst:lua-matchmaker}]
for i = 1, #players do
...
\figph{deployment}{3.0cm}{Per-region Kubernetes resource graph. The
...
The course rules require us to make explicit how the workload was
divided between the two members of the group; the table below
summarises the principal responsibilities. The split is intentionally
based on areas of ownership rather than on a temporal slice: each
member owned the design and implementation of their areas end-to-end,
with cross-review on the report and the architectural decisions.

\begin{table}[h]
  \centering
  \begin{tabular}{@{}p{6.3cm}p{2.3cm}p{2.3cm}@{}}
    \toprule
    \textbf{Area} & \textbf{Marco} & \textbf{Valerio} \\
    \midrule
    Backend application skeleton, Laravel + Octane + FrankenPHP setup & primary & review \\
    Game engine (\texttt{ScopaEngine}, scoring, Santi mechanic, replay) & primary & contribution to Santi/scoring \\
    Matchmaking service and Lua atomic algorithm & primary & review \\
    Persistence schema and YugabyteDB integration & primary & review \\
    Broadcasting, Soketi configuration, Sentinel wiring & primary & contribution \\
    Godot client, scenes, UI flow & contribution & primary \\
    HTTP/WebSocket client, endpoint selector, failover logic & contribution & primary \\
    Helm chart, K3s deployment, regional values & primary & contribution \\
    GitHub Actions CI/CD pipeline, Tailscale integration & primary & review \\
    Chaos testing, tolerance matrix execution & joint & joint \\
    Report writing & joint & joint \\
    \bottomrule
  \end{tabular}
  \caption{Responsibility matrix. ``Primary'' indicates the principal
  author of the area; ``contribution'' indicates substantial co-work;
  ``review'' indicates pair-programming and pull-request review.}
  \label{tab:workload}
\end{table}
...
design can easily generate \emph{honest-looking but wrong} solutions:
the matchmaking service implemented the right ELO-window algorithm
inside a Lua script that correctly serialises critical operations, but
in the present codebase the \texttt{findMatches} routine is invoked
piggy-backed on join requests rather than from a scheduled background
process; if the queue grows during a quiet HTTP minute, nobody is
running the matcher. We have listed this in \Cref{future-works} as
the most impactful follow-up item.
...
If we had another week we would replace the \texttt{sleep(5)} call in
the round-end controller (which currently blocks an entire Octane
worker for five seconds in the name of UI pacing), restore the
input-validation rule that the action controller is supposed to use
before the engine is invoked, and harden the Redis Sentinel quorum
configuration that proved insufficient in our chaos tests.

\section{Future Works}\label{future-works}

The items below are grouped by category and prioritised by gravity.
The most urgent ones materially affect correctness and would be fixed
first; the lower-priority ones are stylistic or strategic.

\subsection{Correctness}

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Background matchmaker.} \texttt{MatchmakingService::findMatches()}
  is currently only invoked on enqueue paths. A scheduled Artisan
  command running every second on a dedicated pod (or an Octane tick)
  must drive the matcher independently of HTTP traffic.
  \item \textbf{Login validation.} \texttt{AuthController::login()}
  declares the \texttt{username} field as \texttt{email} format and
  therefore rejects every legitimate login. The validation rule must
  be relaxed to a plain string.
  \item \textbf{Initial game status.} \texttt{GameController::create()}
  sets the new game's status to \texttt{PLAYING} even when no second
  player has joined yet; the correct enum value is
  \texttt{WAITING\_FOR\_PLAYERS}.
  \item \textbf{Action input validation.} The validator on
  \texttt{POST /games/\{id\}/action} is currently commented out; a
  \texttt{GameActionRequest} must guard the action notation against
  malformed input before the engine is invoked.
  \item \textbf{Santi purchase/use validation.}
  \texttt{MoveValidator::validateBuyAction()} and
  \texttt{validateUseAction()} unconditionally return \texttt{true}.
  Their predicates must be implemented to reject illegal
  purchases (insufficient blood, invalid card combinations) and
  illegal uses (Santo not in inventory, target ineligible).
  \item \textbf{Round-end blocking sleep.} The five-second
  \texttt{sleep(5)} in the round-end callback inside the action
  controller blocks the Octane worker; it should be replaced with a
  delay enforced by the client (or by a delayed-event broadcast on
  the WebSocket side).
  \item \textbf{Round-1 fairness.} The engine currently always starts
  P1 first in every round; the alternation logic exists but is
  commented out and must be restored.
\end{itemize}

\subsection{Distributed-Systems Depth}
...
\subsection{Engineering Polish}

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Observability.} The Dockerfile pre-installs the
  OpenTelemetry PHP extension but it is not wired to a collector;
  setting up a regional OpenTelemetry Collector + Prometheus + Grafana
  stack would give us per-region SLO dashboards.
  \item \textbf{Rate limiting.} The matchmaking and action endpoints
  are not rate-limited. A simple Laravel \texttt{throttle} middleware
  per user would prevent abuse.
  \item \textbf{Tests for distributed concerns.} We have no
  integration tests that exercise the Lua matchmaker under simulated
  multi-pod contention, no Soketi-level tests for the broadcast bus,
  and no chaos automation for the storage tier.
  \item \textbf{Endpoint configuration override.} The Godot client
  hard-codes the two regional hostnames. An environment-variable
  override (or a config file alongside the binary) would simplify
  testing against staging stacks.
  \item \textbf{Information leak in mutations payload.}
  \texttt{GameState::toPublicView()} exposes the full mutations map
  to both players; it should be filtered down to entries that affect
  cards visible to the requesting player.
  \item \textbf{Bearer token rotation.} The Sanctum token issued at
  registration is never refreshed; long-lived sessions are therefore
  pinned to a single token that, if leaked, grants permanent access.
  \item \textbf{Dead code removal.} The \texttt{routes/ai.php} file
  references a class that does not exist in the repository; the file
  must either be completed or removed.
  \item \textbf{GitHub Issues hygiene.} The course rules ask us to
  use GitHub Issues for discussions, feature requests, and bug
  reports. We tracked issues informally in shared notes and the
  project backlog Markdown files; migrating those to GitHub Issues
  would close the loop on the recommended workflow.
\end{itemize}
